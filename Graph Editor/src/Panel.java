...
public class Panel extends JPanel implements MouseListener, MouseMotionListener {
	...
	private void nodeMenu(MouseEvent e, Node node){
		...
		if(nodeUnderCursor instanceof Node) {
			...
			JMenuItem changeNodeItem1 = new JMenuItem("Change key text");
			popupMenu.add(changeNodeItem1);
			changeNodeItem1.addActionListener((action)->{
				changeNodeKeyText(node);
			});
	...
	private void createNewNode(int currentX, int currentY) {
		try {
			...
			String text = JOptionPane.showInputDialog(this, "Value", "New node", JOptionPane.QUESTION_MESSAGE);
			String text1 = JOptionPane.showInputDialog(this, "Key", "New node", JOptionPane.QUESTION_MESSAGE);
			graph.addNode(new Node(currentX, currentY, Color.GREEN,12, text,text1));
			repaint();
	...
	private void changeNodeText(Node node) {
		String text = JOptionPane.showInputDialog(this, "Value", "Edit node", JOptionPane.QUESTION_MESSAGE);
		try {
	...
	private void changeNodeKeyText(Node node) {
		String text = JOptionPane.showInputDialog(this, "Key", "Edit node", JOptionPane.QUESTION_MESSAGE);
		try {
			((Node)node).setText1(text);
			repaint();
		}catch(ClassCastException e) {
			JOptionPane.showMessageDialog(this, "Error", "Error", JOptionPane.INFORMATION_MESSAGE);
		}catch (NullPointerException e) {
